...
As seguintes linhas do arquivo de dados original (`r length(pess_linhas_problemas_1)` no total) apresentaram problemas desta natureza. Alguns exemplos desse desse tipo de ocorrência são:
...
No todo, foram encontrados `r length(pess_linhas_problemas_2)` desse tipo. São exemplos desta ocorrência:
...
A codificação da variável "cem_tipofamilia" será a seguinte:

- **1**  - Familia principal ou unica
- **2**  - Outras familias
- **NA** - Nao se aplica (domicilios coletivos)

As linhas sem valores válidos para a variável cem_IDdomicilio devem, deste modo, dizer respeito a famílias conviventes. Por definição, esses registros devem reecber o mesmo ID de domicílios das famílias principais com as quais co-habitam. Para isso, primeiramente, selecionamos esses casos:
...
Por fim, a antiga variável ID converte-se agora em ID apenas das famílias. E já podemos apagar a variável auxiliar "n_linha", criada para este passo:
...
293555. `31   M       1BA33HIA                                 \0046690`
293556. `31 1 80731608050352911557059200031100000000034        \0046690`
...
Por fim, re-ordenamos as variáveis e eliminamos da lista da variáveis problemáticas a coluna "REC_TYPE" (que já não consta mais no banco de dados):
