...
# Caracteres de 7 a 16 (10 caracteres) n?o s?o lidos no arquivo de domic?lio. 
# Caracteres de 35 a 54 (20 caracteres) n?o s?o lidos no arquivo de domic?lio. Mas isso esta OK. So ha valores nos registros de individuos
# caractere 32 nao esta sendo lido nos domicilios (1257 espa?os vazios, 173209 valores zero, 1 valor 2)
...
# V121 - PESO DOMIC?LIO
# V122 - FILLER
# -- A principio, ? possivel calcular V119 e V120 a posteriori.
...
# -- O conteudo da v122 ? desconhecido (mas provavelmente nao ser trata de nada substantivo)

# As variaveis de identificacao listadas no inicio do dicionario tamb?m nao estao sendo abertas na sintaxe:
# em formato SPSS
# 1 - PASTA 
# 2 - BOLETIM
# 3 - IDENTIFICA??O
...
# As vari?veis UF (unidade da federacao) e RECD (tipo de registro) nao estao listadas no dicionario, mas s?o referenciadas na sintaxe
# Segundo o dicionario para a amostra de 25%, se a variavel "3 - IDENTIFICA??O" contiver apenas zeros, entao o registro ? de domicilios.
# Essa n?o ? a mesma l?gica aplicada na sintaxe (onde domic?lios s?o RECD == 1)

# No entanto, cruzando a vari?vel RECD com a vari?vel sobre RELA??O COM O CHEFE, ? poss?vel inferir o significado das alternativas 
...
# O importante ? checar os seguintes aspectos:
# -- Ela de poder ser transformada em numerica sem gerar nenhum missing
# -- N?o Pode haver valores repetidos (afinal se trata de um ID)
...
# Compilando num objeto todas as linhas detectadas como problem?ticas
...
# -- SE v101 = coletivo, v102 ? vazia
# -- SE v101 = 2a/3a familia ou boletim individual, v102 ? vazia
...
# Essa suposicao ? baseada na sintaxe (SPSS) que acompanhava o arquivo de dados que recebi. A principio, teria
...
censo1960_pess$contains_invalid <- censo1960_pess %>% 
        select(starts_with("test_")) %>%                          # A princ?pio, estava excluindo propositalmente a V217 e V218 (total de filhos tidos e vivos). 
                                                                  # O dicionario dessas variaveis parece conter erros. O valor 2 nao esta listado. 
                                                                  # E h? pessoas com mais de 30 filhos.
        mutate_all(function(x) as.numeric(!(x))) %>%
